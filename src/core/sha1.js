...
function sha1Login(){
    var b=document.getElementById("ba_loginbut");
    if(!b)return;
    b.addEventListener("click",function(e){
        e.preventDefault();
        e.stopPropagation();
        var f=b.closest("form"),s=f&&f.querySelector('input[name="ba_seed"]');
        var p=f&&f.querySelector('input[name="ba_password"]');
        var p2=document.getElementById("ba_password2")||p;
        if(f&&s&&p&&p2){
            var r=f.querySelector('input[name="ba_realm"]'),h;
            if(r&&r.value){ // Calc HA1: https://realtimelogic.com/ba/doc/en/lua/auth.html
                var m=new SparkMD5(),u=f.querySelector('input[name="ba_username"]');
                m.append(utf8Encode(u?u.value:""));
                m.append(":");
                m.append(utf8Encode(r.value));
                m.append(":");
                m.append(utf8Encode(p2.value));
                h=SHA1.hash(m.end()+s.value);
            }
            else h=SHA1.hash(utf8Encode(p2.value)+s.value);
            p.value="";
            p.remove();
...
document.readyState==="loading"?document.addEventListener("DOMContentLoaded",sha1Login):sha1Login();
